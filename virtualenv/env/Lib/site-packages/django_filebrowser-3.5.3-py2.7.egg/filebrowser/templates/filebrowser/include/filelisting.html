...
{% for fileobject in page.object_list %}
    ...
    <tr class="grp-row {% cycle 'grp-row-even' 'grp-row-odd' %}{% if fileobject.filetype == "Folder" %} fb_folder{% endif %}">
        ...
        <td class="fb_thumbnail">
            ...
            {% elif fileobject.filetype == "Mesh" %}
                <img src = "{% static "filebrowser/img/teeth_icon.jpg" %}" alt = "{%trans 'Teeth Icon'}" > 
            {% endif %}
        </td>
		...
		<td>
			<ul >
                {% if fileobject.filetype == "Mesh" %}
                   <a href="{% url 'filebrowser:model_editor' %}{% query_string "" "p" %}&amp;filename={{ fileobject.filename|urlencode }}"><strong>{% trans "View" %}</strong></a>
                {% else %}
                    <span class="fb_type none">&mdash;</span>
                {% endif %}
            </ul>
